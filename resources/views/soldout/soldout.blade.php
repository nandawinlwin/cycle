...
@foreach($shops as $shop)
  @if($shop->id == $auser->shop_id)
    <a class="btn btn-sm btn-primary" href="{{action('SaleController@soldout',$auser->id)}}">{{$shop->name}} <span class="badge text-success">Now</span></a>
  @else
    <a class="btn btn-sm btn-primary" href="{{action('SaleController@soldout',$shop->id)}}">{{$shop->name}} <span class="badge text-primary">Other</span></a>
  @endif
@endforeach
</div>
